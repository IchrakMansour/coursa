...
// Formulaire d'ajout de catégorie (le livreur gère le menu du restaurant)
export function AddCategorieForm({ restaurantId }: { restaurantId: string }) {
  return (
    <form
      action={(fd) => ajouterCategorie(restaurantId, fd)}
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3">
      ...
      <div className="flex shrink-0 items-center gap-3">
        ...
        <button
          disabled={pending}
          onClick={() => startTransition(() => supprimerProduit(restaurantId, produit.id))}
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between">
        ...
        <button
          disabled={pending}
          onClick={() => startTransition(() => supprimerCategorie(restaurantId, categorie.id))}
      ...
      <div className="mt-2 divide-y divide-slate-100">
        {produits.length === 0 ? (
          ...
          produits.map((p) => (
            <ProduitRow key={p.id} restaurantId={restaurantId} produit={p} />
          ))
        )}
      </div>
